/**
 * Created by lixifeng on 16/10/31.
 */
...
class MsgListPage extends React.Component {
    ...
    componentDidMount() {
        ...
        this.iScrollInstance = new iScroll(window.document.getElementById("abc"), options);
    ...
    fetchItems(isRefresh) {
        ...
        if (isRefresh) {    // 刷新操作
            if (this.state.pullDownStatus == 3) {
                ...
                this.iScrollInstance.scrollTo(0, -1 * this.refs.PullDown.getBoundingClientRect().height, 500);
            }
        } else {    // 加载操作
        ...
        ++this.page;
        log(`fetchItems=effected isRefresh=`+isRefresh);
    }
    /**
     * 点击跳转详情页
     */
    ...
    onScroll() {
        let pullDown = this.refs.PullDown.getBoundingClientRect;
    ...
    onScrollEnd() {
        log("onScrollEnd" + this.state.pullDownStatus);

        let pullDown = this.refs.PullDown.getBoundingClientRect();
        // 滑动结束后，停在刷新区域
        if (this.iScrollInstance.y > -1 * pullDown.height) {
            if (this.state.pullDownStatus <= 1) {   // 没有发起刷新,那么弹回去
                this.iScrollInstance.scrollTo(0, -1 * pullDown.height, 200);
    ...
    render() {
        ...
        this.state.items.forEach((item, index) => {
            lis.push(
                <li className={css.li} key={index} onClick={this.onItemClicked}>
                    aa:{item}{index}
                </li>
            );
        })
        ...
        return (
            <div className={css.ScrollContainer}>
                <div id="abc"
                    ...
                    <ul className={css.ListInside}>
                        <p ref="PullDown" className={css.PullDown}>{this.pullDownTips[this.state.pullDownStatus]}</p>
                        {lis}
                        <p ref="PullUp" className={css.PullUp}>{this.pullUpTips[this.state.pullUpStatus]}</p>
                    </ul>
                </div>

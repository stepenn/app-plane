...
export function nextYear(now, index = 0) {
    ...
    const date = new Date(now.getFullYear() + index, now.getMonth(), now.getDate(),now.getHours(),now.getMinutes(),0);
    return date;
}
...
export function nextMonth(now, index = 0) {
    ...
    const date = new Date(year, month, dayOfMonth,now.getHours(),now.getMinutes(),0);
    return date;
}

export function nextDate(now, index = 0) {
    throwIfInvalidDate(now);
    const date = new Date(now.getFullYear(), now.getMonth(), now.getDate() + index,now.getHours(),now.getMinutes(),0);
    return date;
}
export function nextHour(now, index = 0) {
    throwIfInvalidDate(now);
    const date = new Date(now.getFullYear(), now.getMonth(), now.getDate(),now.getHours()+index,now.getMinutes(),0);
    return date;
    // new Date(yyyy,mth,dd,hh,mm,ss);
}
export function nextMin(now, index = 0) {
    throwIfInvalidDate(now);
    const date = new Date(now.getFullYear(), now.getMonth(), now.getDate(),now.getHours(),now.getMinutes()+index,0);
    return date;
    // new Date(yyyy,mth,dd,hh,mm,ss);
}
